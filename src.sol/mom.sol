...
contract SaiMom is DSThing {
    ...
    // Debt ceiling
    function setCap(uint wad) public payable note auth {
        tub.mold("cap", wad);
    }
    // Liquidation ratio
    function setMat(uint ray) public payable note auth {
        tub.mold("mat", ray);
        uint256 axe = tub.axe();
        uint256 mat = tub.mat();
    ...
    // Stability fee
    function setTax(uint ray) public payable note auth {
        tub.mold("tax", ray);
        uint256 tax = tub.tax();
        require(RAY <= tax);
    ...
    // Governance fee
    function setFee(uint ray) public payable note auth {
        tub.mold("fee", ray);
        uint256 fee = tub.fee();
        require(RAY <= fee);
    ...
    // Liquidation fee
    function setAxe(uint ray) public payable note auth {
        tub.mold("axe", ray);
        uint256 axe = tub.axe();
        uint256 mat = tub.mat();
        require(axe >= RAY && axe <= mat);
    ...
    // Join/Exit Spread
    function setTubGap(uint wad) public payable note auth {
        tub.mold("gap", wad);
    }
    // ETH/USD Feed
    ...
    // Boom/Bust Spread
    function setTapGap(uint wad) public payable note auth {
        tap.mold("gap", wad);
        uint256 gap = tap.gap();
    ...
    function setWay(uint ray) public payable note auth {
        require(ray < 1000001100000000000000000000);  // 10% / day
        require(ray >  999998800000000000000000000);
        vox.mold("way", ray);
    }
    function setHow(uint ray) public payable note auth {
        vox.tune(ray);
    }
}
